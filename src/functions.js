...
/* esta función lee mi archivo y regresa una promesa de la data que queda lista
para enlazarse con otra promesa, devuelve el texto listo para la siguiente función */
...
export const petitionHTTP = (link) => new Promise((resolve, reject) => {
  fetch(link, { method: 'HEAD' })
    ...
    .catch((error) => {
      ...
      // console.error(` ${error}, status: ${error.response ? error.response.status : 'desco'}`)
      reject(error);
    });
});
